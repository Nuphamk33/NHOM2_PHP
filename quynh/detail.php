...
<body>
    ...
    <main>
        ...
        }else{ $id=$_SESSION['newsid'];}

        $sql = "SELECT * FROM news 
                join tagnames on tagnames.tagname_id=news.tagname_id
                where news_id='$id'
        ";
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<h5 class="about-pera1 mb-25">#' . (isset($row['content_tag']) ? $row['content_tag'] . '<br><br>' : '') . '</h5>';
            echo '<h7 class="about-pera1 mb-25">Ngày đăng: ' . (isset($row['date']) ? $row['date'] . '<br><br>' : '') . '</h7>';
        ...
        $sql1 = "SELECT comments.content_cmt,users.full_name,comments.time_cmt FROM comments 
                 join users on users.user_id=comments.user_id
                 where comments.news_id='$id' and comments.status_cmt='Hiện'
                 ";
$nameid=$_SESSION['user_id'];
$sql2 = "SELECT comments.content_cmt,users.full_name FROM comments 
join users on users.user_id=comments.user_id
where comments.news_id='$id' and comments.status_cmt='Ẩn' and users.user_id='$nameid'
";
echo "<div style=pading: 20px;>";
echo "<h4 style=color:darkgrey;>Bình luận đang chờ xét duyệt</h4>";         
...
if ($result2->num_rows > 0) {
    ...
    echo '<h6 style=color:darkgrey;>' . (isset($row2['full_name']) ? $row2['full_name'] : '') . ' : ';
        echo '' . (isset($row2['content_cmt']) ? $row2['content_cmt'] : '') . '.';
    echo'</h6>';
    
               
    }
}
echo "<hr 2px solid black>";
        ...
        if ($result1->num_rows > 0) {
            ...
            echo '<div>';     
            echo '<h6><strong>' . (isset($row['full_name']) ? $row['full_name'] : '') . '</strong> ( '.(isset($row['time_cmt']) ? $row['time_cmt'] : '').') : ';
                echo '<p >' . (isset($row['content_cmt']) ? $row['content_cmt'] : '') . '.</p>';
            echo'</h6>';
            echo '</div>';           
            }
