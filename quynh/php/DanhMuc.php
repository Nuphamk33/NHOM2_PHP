...
class DM
{
    ...
    {   $conn = mysqli_connect('127.0.0.1:3308','root','','webtintuc_n2') or die('connection failed');
        ...
        if(mysqli_num_rows($select_DM) > 0){
            ...
            $DMid= $_SESSION['DMid'];
            $select_new = mysqli_query($conn, "SELECT title FROM news WHERE category_id = '$DMid' and status_news='Hoạt động' ") or die('query failed');
            ...
            $DMid= $_SESSION['DMid'];
            $select_new = mysqli_query($conn, "SELECT title ,image,news_id FROM news WHERE category_id = '$DMid' and status_news='Hoạt động' ") or die('query failed');
